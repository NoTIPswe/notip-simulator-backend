...
func TestIntegration_Middleware_EmptyToken_AllowsAll(t *testing.T) {
	// newIntegrationEnv does not set a token → all requests pass through.
	e := newIntegrationEnv(t)
	...
	if resp.StatusCode == http.StatusUnauthorized {
		t.Error("requests should not be blocked when SimTokenSecret is empty")
	}
}

func TestIntegration_Middleware_WithToken(t *testing.T) {
	const token = "super-secret"

	// Minimal mux — we only need one route to exercise the middleware.
	mux := http.NewServeMux()
	mux.HandleFunc("GET /sim/gateways", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
	})
	mux.HandleFunc("GET /health", func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
	})
	srv := httptest.NewServer(httpadapter.SimTokenMiddleware(token, mux))
	t.Cleanup(srv.Close)
	...
	t.Run("health endpoint bypasses token check", func(t *testing.T) {
		...
		defer func() { _ = resp.Body.Close() }()
		if resp.StatusCode != http.StatusOK {
			t.Errorf("expected 200 on /health without token, got %d", resp.StatusCode)
		}
	})
}
